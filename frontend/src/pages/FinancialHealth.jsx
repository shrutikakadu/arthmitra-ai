      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px", width: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <h1 style={{ fontSize: "36px", fontWeight: 800, color: "#0f172a", margin: "0 0 10px 0" }}>
            Family Financial <span style={{ color: "#138808" }}>Health Score</span>
          </h1>
          <p style={{ fontSize: "16px", color: "#64748b", maxWidth: 600, margin: "0 auto" }}>
            Comprehensive 5-dimension evaluation of your household's social security, scheme coverage, and savings potential.
          </p>
        </div>

        {loading ? (
          <Loader text="Calculating 5-Dimension Health Score..." />
        ) : data ? (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 24, alignItems: "start" }}>
            ...
            <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "24px", boxShadow: "0 4px 20px rgba(0,0,0,0.03)", textAlign: "center" }}>
              <h3 style={{ fontSize: "18px", fontWeight: 700, color: "#0f172a", margin: "0 0 12px 0" }}>Overall Health Rating</h3>
              <ScoreGauge score={data.score} grade={data.grade} label={data.score >= 70 ? "Good Standing" : "Moderate Standing"} />
              <p style={{ fontSize: 13, color: "#475569", lineHeight: 1.5, marginTop: 12 }}>
                {data.summary}
              </p>
            </div>

            {/* Dimension Breakdown & Roadmap */}
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "24px", boxShadow: "0 4px 20px rgba(0,0,0,0.03)" }}>
                <h3 style={{ fontSize: "18px", fontWeight: 700, color: "#0f172a", margin: "0 0 16px 0" }}>5-Dimension Score Breakdown</h3>
              ...
              {data.roadmap && data.roadmap.length > 0 && (
                <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "24px", boxShadow: "0 4px 20px rgba(0,0,0,0.03)" }}>
                  <h3 style={{ fontSize: "18px", fontWeight: 700, color: "#0f172a", margin: "0 0 16px 0" }}>🚀 Personalised Improvement Roadmap</h3>
                    ...
                    {data.roadmap.map((item, idx) => (
                      <div key={idx} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderRadius: 8, border: "1px solid #f1f5f9", background: "#fafaf8" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 4, background: item.priority === "HIGH" ? "#fef2f2" : "#f0fdf4", color: item.priority === "HIGH" ? "#ef4444" : "#166534" }}>
                            {item.priority}
                          </span>
                          <span style={{ fontSize: 13, color: "#334155", fontWeight: 500 }}>{item.action}</span>
                        </div>
                        <span style={{ fontSize: 12, fontWeight: 700, color: "#138808", background: "rgba(19, 136, 8, 0.08)", padding: "2px 8px", borderRadius: 12 }}>
                          {item.impact}
                        </span>
                      </div>
                    ))}
