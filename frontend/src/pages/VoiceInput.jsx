...
export default function VoiceInput() {
  ...
  const startListening = () => {
    ...
    recognition.onstart = () => {
      setIsListening(true);
      setTranscript("Listening... Speak your profile details (e.g. 'I am a 35 years old farmer from Maharashtra earning 60000').");
    };
  ...
  const processVoiceText = async (text) => {
    ...
    try {
      ...
      if (res.data.status === "ok" && res.data.profile) {
        ...
        // Auto-run scheme matcher with parsed profile
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px", width: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(168, 85, 247, 0.08)", border: "1px solid rgba(168, 85, 247, 0.2)", padding: "4px 14px", borderRadius: 20, color: "#a855f7", fontSize: 13, fontWeight: 600, marginBottom: 12 }}>
            🎙️ Voice-Enabled NLP Assistant
          </div>
          <h1 style={{ fontSize: "36px", fontWeight: 800, color: "#0f172a", margin: "0 0 10px 0" }}>
            Speak in Your <span style={{ color: "#a855f7" }}>Language</span>
          </h1>
          <p style={{ fontSize: "16px", color: "#64748b", maxWidth: 600, margin: "0 auto" }}>
            No typing required. Speak naturally in English, Hindi, or regional accents to extract your profile and discover eligible government schemes.
          </p>
        </div>
        ...
        <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "36px 24px", boxShadow: "0 4px 20px rgba(0,0,0,0.03)", textAlign: "center", marginBottom: 32 }}>
          <button
            ...
            style={{
              ...
              height: 90,
              borderRadius: "50%",
              background: isListening ? "linear-gradient(135deg, #ef4444, #f97316)" : "linear-gradient(135deg, #a855f7, #6366f1)",
              border: "none",
              color: "#ffffff",
              fontSize: 36,
              cursor: "pointer",
              boxShadow: isListening ? "0 0 0 12px rgba(239, 68, 68, 0.2)" : "0 8px 24px rgba(168, 85, 247, 0.35)",
          ...
          <p style={{ fontSize: 14, color: "#64748b", maxWidth: 450, margin: "0 auto" }}>
            Example: <em>"I am a 35 years old farmer from Maharashtra, OBC category, income 60000 rupees and 4 family members"</em>
          </p>
        </div>
        ...
        {parsedProfile && (
          <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "20px", marginBottom: 32 }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: "#0f172a", margin: "0 0 14px 0" }}>🧠 NLP Extracted Profile Fields</h3>
        ...
        {loading && <Loader text="Running NLP extraction and matching schemes..." />}

        {/* Matched Schemes */}
        {matchedSchemes && (
          <div>
            <h3 style={{ fontSize: "22px", fontWeight: 800, color: "#0f172a", marginBottom: 20 }}>
              Matched Schemes ({matchedSchemes.total_schemes})
            </h3>
            
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 20 }}>
              {(matchedSchemes.schemes || []).map((s, idx) => (
