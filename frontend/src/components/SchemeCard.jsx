...
export default function SchemeCard({ scheme, onClick }) {
  ...
  const handleClick = () => {
    if (onClick) onClick(scheme);
    else navigate(`/scheme/${encodeURIComponent(schemeName)}`);
  };

  return (
    <div onClick={handleClick}
      style={{ background: "#ffffff", border: "1.5px solid #ede8e1", borderLeft: `5px solid ${match >= 85 ? "#138808" : "#FF6B00"}`, borderRadius: "12px", padding: "20px", cursor: "pointer", transition: "all 0.2s ease-in-out", boxShadow: "0 2px 8px rgba(0,0,0,0.02)", display: "flex", flexDirection: "column", justifyContent: "space-between" }}
      ...
      <div>
        ...
        <div style={{ fontSize: "12px", color: "#64748b", marginBottom: 8 }}>
          {t("card_category")}: <span style={{ fontWeight: 600, color: "#334155" }}>{category}</span> • {t("card_benefit")}: <strong style={{ color: "#138808" }}>{benefit}</strong>
        </div>

        {reasons.length > 0 && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}>
            {reasons.slice(0, 3).map((r, i) => (
              <span key={i} style={{ fontSize: "11px", background: "#f8fafc", border: "1px solid #e2e8f0", padding: "2px 8px", borderRadius: 4, color: "#475569" }}>✓ {r}</span>
            ))}
          </div>
        )}
      </div>
